...
std::ostream &operator <<(std::ostream &ispis, const Sat &s){
    ispis<<std::setfill('0')<<std::setw(2)<<s.sati<<":"<<std::setfill('0')<<std::setw(2)<<s.minute<<":"<<std::setfill('0')<<std::setw(2)<<s.sekunde;
    return ispis;
}
Sat operator - (const Sat &s, int p){
    Sat pom; 
    pom.sati=s.sati; pom.minute=s.minute; pom.sekunde=s.sekunde;
    pom.sekunde-=p;
