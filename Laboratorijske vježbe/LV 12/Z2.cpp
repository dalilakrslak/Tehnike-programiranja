...
std::set<Tip> &operator *=(std::set<Tip> &s1, std::set<Tip> s2){
    ...
    std::set<Tip> pom;
    while(p1!=k1){
        if(s2.find(*p1)!=s2.end()) pom.insert(*p1);
        p1++;
    }
    s1=pom;
    return s1;
...
std::ostream &operator <<(std::ostream &ispis, std::set<Tip> skup){
    ispis<<'{';
    auto p(skup.begin());
    if(skup.size()==0) return ispis<<'}';
    for(auto i=0; i<skup.size()-1; i++) ispis<<*p++<<",";
    ispis<<*p<<'}';
    return ispis;
}
template <typename Tip1, typename Tip2>
std::ostream &operator <<(std::ostream &ispis, std::set<std::pair<Tip1,Tip2>> skup){
    ispis<<'{';
    auto p(skup.begin());
    if(skup.size()==0) return ispis<<"}";
