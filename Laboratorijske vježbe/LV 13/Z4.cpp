...
public:
    ...
    void UpisiStudenta(std::string ime, std::string prezime, int indeks){
        for(int i=0; i<v.size(); i++){
            if(indeks==v[i]->DajBrojIndeksa()) throw std::logic_error("Student sa zadanim brojem indeksa vec postoji");
        }
    ...
    void ObrisiStudenta(int indeks){
        for(int i=0; i<v.size(); i++){
            if(indeks==v[i]->DajBrojIndeksa()) break;
            if(i==v.size()-1 && indeks!=v[i]->DajBrojIndeksa()) throw std::logic_error("Student sa zadanim brojem indeksa ne postoji");
        }
        int mjesto(0);
        for(int i=0; i<v.size(); i++){
    ...
    void IspisiSveStudente() const{
        ...
        for(int i=0; i<pomocni.size(); i++) pomocni[i]->IspisiPodatke();
    }
};
class Student{
    ApstraktniStudent *s;
public:
    ...
    Student &operator =(const Student &s1){
        ...
        if(s1.s!=nullptr) pomoc=s1.s->DajKopiju();
        delete s;
        s=pomoc;
        return *this;
    }
