...
public:
    ...
    Biblioteka(const Biblioteka &biblioteka){
        ...
        while(pocetak1!=kraj1){
            mapa_korisnika.insert({pocetak1->first, new Student(*pocetak1->second)});
            pocetak1++;
        }
        auto pocetak2=biblioteka.mapa_knjiga.begin();
        auto kraj2=biblioteka.mapa_knjiga.end();
        while(pocetak2!=kraj2){
            mapa_knjiga.insert({pocetak2->first, new Knjiga(*pocetak2->second)});
            pocetak2++;
        }
    }
    ...
    ~Biblioteka(){
        auto pocetak1=mapa_korisnika.begin();
        auto kraj1=mapa_korisnika.end();
        while(pocetak1!=kraj1){
            delete pocetak1->second;
            pocetak1++;
        }
        auto pocetak2=mapa_knjiga.begin();
        auto kraj2=mapa_knjiga.end();
        while(pocetak2!=kraj2){
            delete pocetak2->second;
            pocetak2++;
        }
    }
    void RegistrirajNovogStudenta(int indeks, std::string ime, std::string prezime, int godina, std::string adresa, std::string telefon){
        ...
        Student *novi_student=new Student{indeks, godina, ime, prezime, adresa, telefon};
    ...
    void RegistrirajNovuKnjigu(int broj, std::string naslov, std::string pisac, std::string zanr, int izdanje){
        ...
        Knjiga *nova_knjiga=new Knjiga{naslov, pisac, zanr, izdanje};
